...
class NetworkTrafficMonitor @Inject constructor(
    ...
    private suspend fun updateUnitTextSizeLocked() {
        ...
        logD {
            "defaultTextSize = $defaultTextSize, size = ${state.size}"
        }
        notifyCallbacksLocked()
    }
    ...
    private suspend fun notifyCallbacksLocked() {
        logD {
            "notifying callbacks about new state = $state"
        }
    ...
    private fun scheduleJob() {
        if (trafficUpdateJob != null) {
            logD {
                "Job is already scheduled, returning"
            }
            return
        }
        if (!networkAvailable) {
            logD {
                "No active connection, returning"
            }
            return
        }
        if (isDozing) {
            logD {
                "Device is dozing, returning"
            }
            return
        }
        logD {
    ...
    private fun cancelScheduledJob() {
        if (trafficUpdateJob == null) {
            logD {
                "Job is already cancelled, returning"
            }
            return
        }
        logD {
    ...
    private fun createNewJob(): Job {
        return coroutineScope.launch(Dispatchers.IO) {
            ...
            do {
                ...
                logD {
                    "rxBytes = $rxBytes, currentRxBytes = $currentRxBytes, " +
                    "rxTrans = $rxTrans, rxThreshold = $rxThreshold"
                }
                logD {
                    "txBytes = $txBytes, currentTxBytes = $currentTxBytes, " +
    ...
    private fun getRateFormatted(bytes: Long): SpannableString {
        ...
        logD {
            "bytes = $bytes, rate = $rate, rateString = $rateString, unit = $unit"
        }
    ...
    companion object {
        ...
        val TAG = NetworkTrafficMonitor::class.simpleName

        inline fun logD(crossinline msg: () -> String) {
