...
import com.android.systemui.statusbar.policy.NetworkTrafficMonitor.Companion.logD
...
class StatusBarNetworkTrafficView @JvmOverloads constructor(
    ...
    override fun setVisibleState(newVisibleState: Int, animate: Boolean) {
        logD {
            "setVisibleState, newVisibleState = $newVisibleState"
        }
    ...
    fun applyNetworkTrafficState(newState: NetworkTrafficState) {
        logD {
            "applyNetworkTrafficState, state = $state, newState = $newState"
        }
        if (state == null) {
            state = newState.copy()
            initViewState()
    ...
    companion object {
        ...
        fun fromContext(context: Context, slot: String): StatusBarNetworkTrafficView {
            ...
            return (layout as StatusBarNetworkTrafficView).also {
                it.slot = slot
                it.setWidgets()
